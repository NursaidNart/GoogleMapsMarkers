                ...
                <div class="card">
                    ...
                    <form class="row g-3">
                        ...
                        <div class="col-md-6 offset-md-4 my-2">
                            <button class="btn btn-primary" @click.stop.prevent="editUser()">Save</button>
                        </div>
                    </form>
                    </div>
                </div>
...
export default {
    ...
    mounted () {
        ...
        app.getUser(this.$route.params.id);
    },
    methods: {
        getUser(id){
            let app = this;
            this.axios.get('/user/'+id)
                .then(response => {
                    ...
                    if(app.user.permissions){
                        app.user.permissions.forEach(function(permission){
                            app.permissions.push({
                                'key':permission,
                                'value':app.data.permissions[permission]
                            });
                        });
                    }
                    if(app.user.role_type){
                        app.role_type = {
                            'key': app.user.role_type,
                            'value': app.data.roles[app.user.role_type]
                        };
                    }
        ...
        editUser(){
            ...
            let model = app.user;
            this.axios.put('/user/'+app.$route.params.id,model)
                .then(response => {
                    app.show_alert = true;
